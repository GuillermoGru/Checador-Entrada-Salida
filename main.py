...
# ------Actualización de Catálogos e Historiales------
    ...
    # 2. Vamos a la pestaña Variables a buscar la regla de los minutos de tolerancia
    df_datos_actual = db_sheets.get('Datos') 
    margen_minutos = 60
    try:
        df_vars = pd.read_excel(args.base_datos, sheet_name='Variables')
        fila_margen = df_vars[df_vars['Variable'].astype(str).str.contains('Margen', case=False, na=False)]
        if not fila_margen.empty:
            valor_extraido = pd.to_numeric(fila_margen['Valor'].iloc[0], errors='coerce')
            if pd.notna(valor_extraido):
                margen_minutos = int(valor_extraido)
    except:
        pass

    # 3. Procesamos y guardamos la asistencia normal
    df_datos_nuevo = utils.actualizar_datos_historicos(df_datos_actual, df_eventos, margen_minutos)
    ...
    df_datos_veladores_nuevo = utils.actualizar_datos_veladores(df_datos_veladores_actual, df_veladores, margen_minutos)
