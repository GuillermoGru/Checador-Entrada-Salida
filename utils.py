...
def actualizar_datos_historicos(df_existente: pd.DataFrame, df_eventos_nuevos: pd.DataFrame, df_ignorar: pd.DataFrame, margen_minutos: int = 60) -> pd.DataFrame:
    ...
    lista_df = []
    if df_existente is not None and not df_existente.empty:
        #Eliminar registros del historial viejo que caigan en el periodo que estamos reconstruyendo
        ...
        for hora in marcas:
            if entrada_actual is None:
                ...
                if diff.total_seconds() < margen_segundos:
                    ...
                    # Verificamos si entraron y salieron el mismo día calendario
                    if entrada_actual.date() == hora.date():
                        resultados.append({
                            'ID de Empleado': empleado_id,
                            'Fecha': entrada_actual.strftime('%Y-%m-%d'),
                            'Hora entrada': entrada_actual.strftime('%H:%M'),
                        ...
                        resultados.append({
                            ...
                            'Hora entrada': entrada_actual.strftime('%H:%M'),
                            'Hora salida': "",         
                            # Usamos un espacio en blanco (" ") intencionalmente para que reportes.py 
                            # siga calculando retardos de entrada, pero deje vacía la columna "Horas"
                            'Diferencia horas': " " 
                        })
